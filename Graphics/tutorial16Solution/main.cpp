...
private:
	...
	Materials _material1, _material2, _material3, _black;
...
public:
	...
	void OnCreate() {
		...
		_black.create(Color::black(), Color::black(), Color::black());
		...
		_light4.create(1, Color::black(), Color(0.5f,0.5f,0.5f,1.0f)); 
	...
	void BackWall()
	{
		...
		glBegin(GL_QUADS);
			glNormal3f(0.0f,0.0f,1.0f);
			glTexCoord2d(0.0, 0.0);		glVertex3d(-10.0, 0.0,  -10.0);
			glTexCoord2d(1.0, 0.0);		glVertex3d( 10.0, 0.0,  -10.0);
			glTexCoord2d(1.0, 1.0);		glVertex3d( 10.0, 20.0,  -10.0);
			glTexCoord2d(0.0, 1.0);		glVertex3d(-10.0, 20.0,  -10.0);
		glEnd();
		glEnable(GL_TEXTURE_2D);
	}
	...
	void OnDisplay() {
		...
		glPushMatrix();
			...
			glPushMatrix();
				_light2.setPosition(Vector4f(_light2[0],_light2[1],_light2[2],1.0));
				glTranslatef(_light2[0],_light2[1],_light2[2]);
				_material3.apply();
				_sphere1.draw();
			glPopMatrix();
...
// floor shadows
			...
			glPushMatrix();												// now draw the shadow
				glTranslatef(0.0f, 0.1f, 0.0f);							// push it just off the floor
				matrix[0] = _light2[1];									// setup the projection matrix for the shadow (see lecture notes)
				matrix[1]= matrix[2]=matrix[3] = 0.0f;
				matrix[4] = -_light2[0];
				matrix[5] = 0.0f;
				matrix[6] = -_light2[2];
...
// back wall shadows
			glPushMatrix();												// now draw the shadow
				glTranslatef(0.0f, 0.0f, -9.9f);							// push it just off the back wall
				matrix[0] = _light3[2]+10;									// setup the projection matrix for the shadow (see lecture notes)
				matrix[1] = matrix[2] = matrix[3] = matrix[4] = 0.0f;
				matrix[5] = _light3[2]+10;
				matrix[6] = matrix[7] = 0.0f;
				matrix[8] = -_light3[0];
				matrix[9] = -_light3[1];
				matrix[10] = 0.0f;
				matrix[11] = -1.0f;
			...
			glPushMatrix();												// now draw the shadow
				glTranslatef(0.0f, 0.0f, -9.9f);							// push it just off the back wall
				matrix[0] = _light2[2]+10;									// setup the projection matrix for the shadow (see lecture notes)
				matrix[1] = matrix[2] = matrix[3] = matrix[4] = 0.0f;
				matrix[5] = _light2[2]+10;
				matrix[6] = matrix[7] = 0.0f;
				matrix[8] = -_light2[0];
				matrix[9] = -_light2[1];
				matrix[10] = 0.0f;
