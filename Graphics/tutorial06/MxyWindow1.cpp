...
MxyWindow1::MxyWindow1() : _angleInc(30) {
	SetSize(512,512);
	SetDepthBits(24);
...
void MxyWindow1::OnCreate() {
	...
	// setup one cylinder
	_openCylinder.create(0.5f, 0.0f, 0.3f,100, 10);
...
void MxyWindow1::OnResize(int w, int h) {
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
...
void MxyWindow1::OnKeyboard(int key, bool down) {
	if (!down) return;
	switch( tolower(key) ) {
	case 'a': 
		_cameraAngle += 5.0;
...
void MxyWindow1::OnMouseButton(MouseButton button, bool down) {
	if (down) {
		switch(button) {
		case MBLeft: 
			_leftDown = true;
...
void MxyWindow1::OnMouseMove(int x, int y) {
