...
uniform sampler2D TexToUse;

void main() {
	float diffavg = (gl_FrontMaterial.diffuse.r + gl_FrontMaterial.diffuse.g + gl_FrontMaterial.diffuse.b) / 3.0;
	vec4 color = vec4(vec3(texture2D(TexToUse, gl_TexCoord[0].st)),1.0) * diffavg;
